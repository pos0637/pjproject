...
static void on_pager2(pjsua_call_id call_id, const pj_str_t *from,
    ...
    PJ_LOG(3, (THIS_FILE, "on_pager2 MESSAGE from %.*s: %.*s (%.*s)",
               (int)from->slen, from->ptr,
               (int)body->slen, body->ptr,
               (int)mime_type->slen, mime_type->ptr));

    const pj_str_t query = {"<Query>", 7};
    const pj_str_t catalog = {"<CmdType>Catalog</CmdType>", 26};
    if ((pj_strstr(body, &query) != NULL) && (pj_strstr(body, &catalog) != NULL)) {
        const pj_str_t sn1 = {"<SN>", 4};
        const pj_str_t sn2 = {"</SN>", 5};
        const char *p1 = pj_strstr(body, &sn1);
        const char *p2 = pj_strstr(body, &sn2);

        if ((p1 == NULL) || (p2 == NULL)) {
            return;
        }

        const int len = p2 - p1 - 4 + 1;
        char sn[len];
        strncpy(sn, p1 + 4, len);
        sn[len - 1] = '\0';
