...
static void vid_handle_menu(char *menuin) {
    ...
    if (argc == 1 || strcmp(argv[1], "help") == 0) {
        ...
        if (argc == 5 && strcmp(argv[2], "rx") == 0) {
            ...
            pj_bool_t on = (strcmp(argv[3], "on") == 0);
            pjsua_call_vid_strm_op op = on ? PJSUA_CALL_VID_STRM_START_TRANSMIT : PJSUA_CALL_VID_STRM_STOP_TRANSMIT;
            ...
            pjsua_call_vid_strm_op op = enable ? PJSUA_CALL_VID_STRM_CHANGE_DIR : PJSUA_CALL_VID_STRM_REMOVE;
...
static void ui_conf_connect(char menuin[]) {
    ...
    if (cnt != 3) {
        ...
        src_title = (menuin[1] == 'c' ? "Connect src port #" : "Disconnect src port #");
